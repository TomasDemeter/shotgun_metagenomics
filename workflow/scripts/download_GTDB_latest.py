...
try:
    ...
    if os.path.exists('ar53_metadata.tsv'):
        print('Already had the archaea metadata file ar53_metadata.tsv. Move this to a different folder if you want to download this again')
    else:
        os.system('wget -q https://data.gtdb.ecogenomic.org/releases/latest/ar53_metadata.tsv.gz')
        os.system('gunzip ar53_metadata.tsv.gz')
        files = [f for f in os.listdir(os.getcwd()) if 'ar53_metadata' in f and 'tsv' in f][0]
        print('Successfully downloaded and unzipped ar53_metadata.tsv.gz and its now saved as ar53_metadata.tsv')
    if os.path.exists('bac120_metadata.tsv'):
        ...
        os.system('wget -q https://data.gtdb.ecogenomic.org/releases/latest/bac120_metadata.tsv.gz')
        os.system('gunzip bac120_metadata.tsv.gz')
        files = [f for f in os.listdir(os.getcwd()) if 'bac120_metadata' in f and 'tsv' in f][0]
        print('Successfully downloaded and unzipped bac120_metadata.tsv.gz and its now saved as bac120_metadata.tsv\n')   
except:
    print("Failed to download GTDB metadata files. These should be saved as ar53_metadata.tsv.gz and bac120_metadata.tsv.gz. If these aren't downloaded, you can check whether the file paths are dead and modify these in the script. They should be here in on the GTDB website: https://data.gtdb.ecogenomic.org/releases/latest/\n")
    sys.exit()

print('Opening the metadata files')
bac, arc = pd.read_csv('bac120_metadata.tsv', header=0, sep='\t', low_memory=False), pd.read_csv('ar53_metadata.tsv', header=0, sep='\t', low_memory=False)
...
genomes = genomes.groupby(by=genomes.index).first()
new_acc_dict = {}
...
try:
    if download_genomes != True:
        ...
        if os.path.exists('gtdb_genomes_reps/'):
            ...
            for out in output:
              out = out.strip()  # remove newline characters and leading/trailing whitespace
              print(out)
              destination = os.path.join("gtdb_genomes_reps", os.path.basename(out))
              os.system(f"mv '{out}' '{destination}'")
